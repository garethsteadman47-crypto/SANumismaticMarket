 ...
 * Shared by `prisma/seed.ts` (CLI) and `/api/v1/admin/seed-demo` (Vercel/Atlas).
 */
import {
  ListingCategory,
  ListingType,
...
const DEMO_PASSWORD = DEMO_SEED_PASSWORD;
...
export type DemoSeedResult = {
  users: { name: string; email: string; tier: string; isVerified: boolean }[];
  password: string;
  counts: { activeListings: number; liveAuctions: number };
};
